...
def janeRequest(data):
    ...
    for hit in json_data['hits']:
        mary_dir[hit['name']] = {
            'category': hit['category'].capitalize(),
            'brand': hit['brand'],
    ...
    mary_dir = dict(sorted(mary_dir.items(), key=lambda item: item[1]['dpg'][0])[:6])
...
def dutchrequest(dutchID):
    ...
    for product in json_data["data"]["filteredProducts"]["products"]:
        dutch_dir[product["Name"]] = {
            "category": product["strainType"].capitalize(),
        ...
        for offer in range(len(current["prices"])):
            ...
            dpg = round(float(current["prices"][offer]) / float(current["weights"][offer]), 2)
    ...
    dutch_dir = dict(sorted(dutch_dir.items(), key=lambda item: item[1]['dpg'][0])[:6])
